...
assert.equal(out.body.settings.retentionDays, null, 'legacy boolean clients can still select forever')
out = await call('/session-trash/settings', { purgeOnShutdown: true })
assert.equal(out.body.settings.retentionDays, 0, 'legacy boolean clients can still select shutdown purge')
