...
assert.ok(client.includes("'settings.retention.7'"), 'bundle offers a seven-day retention policy')
assert.ok(client.includes("'settings.retention.30'"), 'bundle offers a thirty-day retention policy')
assert.ok(client.includes("'settings.retention.forever'"), 'bundle offers a keep-forever policy')
...
assert.ok(client.includes("'/session-trash/export'"), 'bundle downloads ZIP backups')
assert.ok(client.includes("'/session-trash/history'"), 'bundle exposes metadata-only activity history')
assert.ok(client.includes("'/session-trash/diagnostics'"), 'bundle exposes compatibility diagnostics')
...
assert.ok(client.includes("setStatusFilter"), 'bundle provides status filtering')
assert.ok(client.includes("setSortBy"), 'bundle provides trash sorting')
assert.ok(client.includes("setSelected"), 'bundle provides multi-select actions')
assert.ok(client.includes("formatRemaining"), 'bundle renders expiry countdowns')
assert.ok(client.includes("settings.export.title"), 'bundle exposes export-before-delete policy')
assert.ok(client.includes("settings.space.title"), 'bundle exposes storage warning controls')
assert.ok(client.includes('right.entry.archivedAt - left.entry.archivedAt'), 'trash rows are newest first')
assert.ok(client.includes("summary.origin !== 'subagent' || (summary.parentId && list.byId[summary.parentId])"), 'orphaned subagent summaries are not offered as deletable sessions')
