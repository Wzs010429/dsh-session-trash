...
| 操作 | 效果 |
| --- | --- |
| **删除（Delete）** | 会话立即从侧边栏所有分组和搜索中隐藏（走官方归档机制，所有打开的标签页同步隐藏）；日志仍原样保存在磁盘上；需要勾选确认（二次确认）。 |
...
- `cordis.patch.yml` 通过 bundle patch 把自己的行插入 profile 组合；
- `lib/index.js`（host）：注册 3 个 HTTP 路由（`/session-trash/list|delete|restore`），维护内存回收站 + 磁盘删除清单，注册 `process.on('exit')` 硬删除钩子；
- `lib/client.js`（browser）：向 `sidebar.footer.action` slot 注册「回收站」入口按钮与面板，用官方 `RiskConfirmation`/`Toast`/`Button` 原语渲染，通过 fetch 调用 host 路由。
...
**尚未在真实 `dsh web` 会话中验证 UI 交互**（插件需要重启 web 才能加载，会中断当前会话）——安装后请按下面的清单手动验收，遇到问题在 GitHub 提 issue。

### 手动验收清单

1. 侧边栏底部出现「回收站」按钮，点击展开面板，能看到会话列表；
2. 点「删除」→ 出现确认框，必须勾选确认才能点「删除」；
...
- 会话行菜单（右键 `…`）是 DSH 内置 UI 硬编码的，插件无法向其中注入「删除」项，因此入口放在侧边栏底部的回收站面板。

## 兼容性 Compatibility

- 针对 DSH `0.1.0-rc.x`（web profile）设计与测试；依赖 `dsh-workspace` 的 `enqueueOperation/setState` 内部方法签名与 `host/archived-sessions-changed` 广播机制。DSH 升级若改动这些内部实现，请及时反馈。
