...
> DeepSeek Harness（DSH）Web GUI 的崩溃安全会话回收站：可恢复软删除、预览磁盘占用，并安全执行单项或批量永久清理。

**A crash-safe session trash bin for the DSH web GUI with restore, artifact-size preview, and guarded permanent purge.**
...
5. 回收站显示每项占用；点「永久删除」或「清空回收站」→ 出现独立强确认，运行中会话不会进入清理；
6. 关闭「关闭时永久清理」后重启：回收站内容仍存在并可恢复；
7. 重新开启该策略，正常关闭 `dsh web`（Ctrl+C）后重启：待删除会话彻底消失。
