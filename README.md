...
> DeepSeek Harness（DSH）Web GUI 的完整会话回收站：崩溃安全删除、级联与批量管理、ZIP 备份、操作记录、保留期和空间管理，并适配跨标签页同步与原生浅色/深色主题。
...
| **查找与批量管理** | 可按标题、session ID 或工作区搜索，按状态筛选，并按删除时间、到期时间、大小或名称排序；支持多选恢复、导出和永久删除。 |
...
| **备份与审计** | 单项或级联批次可导出标准 ZIP；可选择“永久删除前必须导出”。最近操作只记录操作类型、时间、session ID、结果和大小，不保存会话正文。 |
| **空间管理** | 显示总占用、未知大小、运行中/到期数量和最大会话；支持 1/5/10 GB 提醒以及安全清理已到期或所有已停止会话，不会提前删除未到期内容。 |
| **兼容诊断** | host 检查归档、恢复、工件定位、运行状态和 metadata 清理能力；面板显示异常并可一键复制诊断信息。 |
...
- `lib/index.js`（host）：**声明 `inject`**，注册 list/preview/delete/restore/settings/purge/export/history/diagnostics 路由，维护批次事务、磁盘占用缓存、ZIP 备份、审计记录和保留/空间策略；有限保留期每小时筛选到期项，正常关停时再次精确筛选；
...
- **备份门禁**：host 生成不依赖第三方库的标准 ZIP，并在清单行记录成功导出时间；开启门禁后，未导出的条目不会进入手动、定时或关停物理删除；
- **隐私受限的操作历史**：`session-trash-audit.json` 最多保留 200 条 metadata 事件，不复制会话内容，可从面板清除；
- **空间策略只提醒**：容量阈值不会覆盖保留期，只提供占用告警和明确的安全清理入口；
...
- **独立策略文件**：v3 `storages/session-trash-settings.json` 保存 `retentionDays`、`requireExportBeforePurge` 和 `spaceWarningBytes`，旧版布尔/v2 设置读取时自动兼容。
...
- `scripts/purgeflow.test.mjs`：验证大小/空间统计、诊断、运行中保护、导出门禁、ZIP、操作历史、批量恢复以及运行时 metadata 同步；
...
- `scripts/runtimesweep.test.mjs`：验证运行期定时清理到期项、续跑崩溃中断的 `purging` 项，并保留未到期项；
- `scripts/clientbundle.test.mjs`：防回归检查，保证发布 bundle 包含撤销与跨标签页同步，同时不嵌入或禁用官方 Workspace，也不会注册 `workspace` locale；
- `.github/workflows/ci.yml`：Node.js 20/22 自动构建、测试、语法检查和 npm 打包检查；
...
8. 验证搜索、六种状态筛选、五种排序、多选恢复/导出/永久删除以及每项到期倒计时；
...
11. 开启「永久删除前必须导出」，确认未导出条目不可永久删除；导出 ZIP 后按钮恢复可用；
12. 设置空间提醒阈值，检查最大会话、到期清理、停止会话清理、最近操作和诊断信息复制。
...
- **浏览器 ZIP 上限为 512 MiB**：当前导出为浏览器内存下载，超过上限会明确拒绝，不会生成不完整备份；大体积会话仍保留且可恢复。
...
- [x] 到期倒计时、状态筛选、排序和多选批量操作
- [x] ZIP 备份与永久删除前导出门禁
- [x] 最近操作记录与兼容诊断
- [x] 空间阈值提醒、最大会话和安全回收入口
- [x] Node 20/22 GitHub Actions 发布前验证
