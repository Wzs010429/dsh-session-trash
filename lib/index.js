 ...
 *
 *  - HARD DELETE (optional on normal shutdown): the plugin observes shutdown
 *    signals and synchronously removes each trashed session's artifact
 *    directory during Cordis disposal, prunes its ids from
 *    `storages/workspace.json` (workspace accounting + archive set), and drops
 *    its rows from `storages/session_projcache.json`. An exit hook remains as
 *    fallback. All metadata writes are tmp-file + rename (atomic).
 *
 ...
 *    and nothing is ever lost by accident. A clean shutdown commits the purge
 *    only when the persisted `purgeOnShutdown` setting is enabled.
 *
  ...
  ctx.effect(() => {
    ...
    const purgeTrash = () => {
      ...
      try {
        ...
        if (metadataClean) {
          for (const sessionId of removed) trashMap.delete(sessionId)
          saveTrash()
