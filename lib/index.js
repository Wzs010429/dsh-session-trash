...
export function normalizeSettings(doc) {
  if (doc?.retentionDays === null || RETENTION_DAYS.has(doc?.retentionDays)) {
    return { retentionDays: doc.retentionDays }
  }
  return { retentionDays: doc?.purgeOnShutdown === false ? null : 0 }
}
...
export function apply(ctx) {
  ...
  const runRetentionSweep = async () => {
    ...
    try {
      const result = await purgeEntries(expired)
  ...
  if (webServer) {
    ...
    register(`${API_ROOT}/settings`, async (req, res) => {
      ...
      let retentionDays
      if (body.retentionDays === null || RETENTION_DAYS.has(body.retentionDays)) retentionDays = body.retentionDays
      else if (typeof body.purgeOnShutdown === 'boolean') retentionDays = body.purgeOnShutdown ? 0 : null
      else return respond(res, 400, { error: 'retentionDays must be 0, 7, 30, or null' })
      try {
        saveSettings({ retentionDays })
      } catch (error) {
  ...
  ctx.effect(() => {
    ...
    const purgeTrash = () => {
      ...
      purging = true
      try {
        const result = purgeEntries(expired, { allowLive: true, runtimeMetadata: false })
