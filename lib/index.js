...
export function apply(ctx) {
  /** in-memory trash: sessionId -> { sessionId, archivedAt, artifactPath, state } */
  ...
  const loadTrash = () => {
    ...
    for (const entry of doc.trash) {
      ...
      if (normalized && !trashMap.has(normalized.sessionId)) trashMap.set(normalized.sessionId, normalized)
    }
  }
  loadTrash()
  ...
  if (webServer) {
    ...
    register(`${API_ROOT}/delete`, async (req, res) => {
      ...
      const entry = { sessionId, archivedAt: Date.now(), artifactPath: undefined, state: 'pending' }
      try {
