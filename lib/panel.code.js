		...
		function TrashEntry(props) {
			...
			const [settings, setSettings] = React.useState({ retentionDays: 0 })
			...
			const updateRetentionPolicy = async (retentionDays) => {
				setBusy(true)
				try {
					const data = await callApi('/session-trash/settings', { retentionDays })
					if (data.settings && (data.settings.retentionDays === null || [0, 7, 30].includes(data.settings.retentionDays))) setSettings(data.settings)
							...
							React.createElement(
								...
								React.createElement(
									'select',
									{
										...
										onChange: (event) => updateRetentionPolicy(event.target.value === 'forever' ? null : Number(event.target.value)),
									},
