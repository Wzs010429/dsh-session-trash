		...
		function UndoToast({ sequence, text, actionLabel, disabled, onUndo, onDone }) {
			...
			return ReactDOM.createPortal(
				React.createElement(
					'div',
					{ style: styles.undoToast, role: 'status', 'aria-live': 'polite' },
		...
		const styles = {
			...
			undoToast: { position: 'fixed', zIndex: 1000, top: 24, left: '50%', transform: 'translateX(-50%)', width: 'max-content', maxWidth: 'calc(100vw - 32px)', boxSizing: 'border-box', display: 'flex', alignItems: 'center', gap: 14, padding: '10px 12px', borderRadius: 10, color: 'var(--dsw-alias-label-primary, #1f2328)', background: 'var(--dsw-alias-surface-raised, #ffffff)', border: '1px solid var(--dsw-alias-divider, rgba(127,127,127,0.2))', boxShadow: '0 12px 32px rgba(0,0,0,0.24)', fontSize: 13 },
			undoText: { minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
			undoButton: { flexShrink: 0, border: 'none', padding: '3px 5px', borderRadius: 5, color: 'var(--dsw-alias-accent-primary, #2563eb)', background: 'transparent', font: 'inherit', fontWeight: 700, cursor: 'pointer' },
		...
		function TrashEntry(props) {
			...
			const doDelete = async () => {
				...
				try {
					...
					setTrash(Array.isArray(data.trash) ? data.trash : [])
					setToast(null)
					setUndo({ key: Date.now(), sessionId: target.id, text: t('toast.undo.description') })
					publishChange()
				} catch (error) {
