 ...
 *   2. a fork of the stock workspace browser (C:/Users/ROG/AppData/Roaming/npm/node_modules/@deepseek-ai/dsh/node_modules/@deepseek-ai/dsh-client-ui-workspace/lib/client.js, 0.1.0-rc.6)
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onDelete, onArchive, drag, flat = false, t }) {
			const row = node;
		...
		*/
		function WorkspaceBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, searchSessions, searchResultLimit, useDirectoryFlow, renderSlot, t }) {
			...
			const confirmSessionDelete = async () => {
				...
				try {
					const response = await fetch("/session-trash/delete", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({ sessionId: sessionDeleteTarget.sessionId })
					});
					if (!response.ok) throw new Error("HTTP " + response.status);
					setSessionToast({ key: Date.now(), text: t("menu.deleteToast") });
				} catch {
					setSessionToast({ key: Date.now(), text: t("menu.deleteFailed") });
				} finally {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.RiskConfirmation, {
						...
						description: sessionDeleteTarget === null ? "" : t("menu.deleteDesc", { title: sessionDeleteTarget.title }),
						acknowledgeLabel: t("menu.deleteAck"),
						cancelLabel: t("cancel"),
						confirmLabel: t("menu.deleteSession"),
						acknowledged: sessionDeleteAck,
						disabled: sessionDeleting,
						onAcknowledgedChange: setSessionDeleteAck,
						onCancel: closeSessionDelete,
						onConfirm: confirmSessionDelete
					}),
					sessionToast !== null && (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Toast, {
						key: sessionToast.key,
						text: sessionToast.text,
						onDone: () => {
