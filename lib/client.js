/**
 * @dsh-external/dsh-session-trash — client half.
 *
 * Registers a trash-bin entry into the sidebar footer (`sidebar.footer.action`,
 * list kind) and a self-contained panel (body portal) that lists deletable
 * sessions and the trash. Delete goes through a RiskConfirmation dialog
 * (secondary confirmation) and calls the host half over its private HTTP
 * routes. Restore reverses the soft delete while the web process is alive.
 *
 * The bundle is a hand-written loader factory: `react`, `react-dom` and the
 * UI primitives are resolved from the shell's frozen module table, so no
